local function add_udev_rules()
	...
	local mavsign_user = require "__mvrk.mavsign.user".username
	...
	ami_assert(user_plugin.add_into_group(mavsign_user, "plugdev"), "failed to add user '" .. mavsign_user .. "' to plugdev")

	local tmp_file_path = os.tmpname()
	local udev_rules_url =
	"https://raw.githubusercontent.com/mavryk-network/mavsign/refs/heads/main/tools/add_udev_rules.sh"
...
local function setup(options)
	if not options["platform"] then
		log_debug("Skipping mavsign/platform setup...")
		return
	end
	...
	log_info("Configuring mavsign for platform: " .. tostring(system_os))
	...
	log_success("Ledger mavsign setup completed.")
end
