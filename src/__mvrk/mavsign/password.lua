...
local function setup_password(option)
    if not option["password"] then
        log_debug("Skipping mavsign password prompt...")
        return
    end
    -- Usage
    local pwd, err = read_password("Enter MavSign Unlock Password: ")
    ami_assert(pwd, "Failed to read password: " .. tostring(err))
    log_info("MavSign password captured (len): " .. #pwd)

    local ok = fs.write_file("mavsign.secret", pwd, { atomic = true })
    ami_assert(ok, "Failed to write mavsign.secret file!")
