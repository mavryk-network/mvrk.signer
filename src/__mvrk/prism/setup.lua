...
local prism_key_generator = require"__mvrk.prism.key_generator"

fs.mkdirp("prism/keys")
fs.mkdirp("prism/conf.d") -- not needed right now

fs.copy("__mvrk/prism/assets", "prism", { overwrite = true })

local ok, err = prism_key_generator.generate("prism/keys/signer", "mavryk-signer")
ami_assert(ok, err or "failed to generate signer key")

local ok, err = prism_key_generator.generate("prism/keys/node", "mavryk-node")
ami_assert(ok, err or "failed to generate signer key")

local ok, err = prism_key_generator.generate("prism/keys/dal", "mavryk-dal")
ami_assert(ok, err or "failed to generate signer key")
